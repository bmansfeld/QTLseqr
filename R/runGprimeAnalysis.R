...
#' 1) The G statistic is culculated by \link{\code{getG}} statisic G is defined
#' by the equation: \deqn{G = 2*\sum_{i=1}^{4} 
...
#' + n_4))/(n_1 + n_2 + n_3 + n_4)} etc... 2) \link{\code{tricubeGStat}} uses
#' local regression to predict a tricube smoothed version of the G statistc for
#' each SNP. This works as a weighted average across neighboring SNPs that
#' accounts for Linkage disequilibrium (LD) while minizing noise attributed to
#' SNP calling errors. G values for neighboring SNPs within the window are
#' weighted by physical distance from the focal SNP. 3) \link{\code{getPvals}}
#' then estimates p-values for the weighted G' statistic based on the
#' non-parametric estimation method described in
#' \href{http://journals.plos.org/ploscompbiol/article?id=10.1371/journal.pcbi.1002255}{Magwene
#' et al. 2013}. Breifly, using the natural log of Gprime a median absolute
#' deviation (MAD) is calculated. The Gprime set is trimmed to exclude outlier
#' regions (i.e. QTL) based on Hampel's rule. An alternate method for filtering
#' out QTL is proposed using absolute delta SNP indeces greater than 0.1 to
#' filter out potential QTL.An estimation of the mode of the trimmed set is
#' calculated using the \code{\link[modeest]{mlv}} function from the package 
...
#'   columns. G - The G statistic for each SNP
