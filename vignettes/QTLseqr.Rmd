...
1. `runQTLseqAnalysis` - performs Takagi et al tyep QTLseq analysis
...
1. The minium read depth at each position is calculated and the tricube-smoothed depth is calculated for the window.
...
While `getSigRegions` is useful for examining every SNP within each QTL and perhaps for some downstream analysis, the `getQTLTable` will summarize those results and can output a CSV by setting `export = TRUE` and `fileName = "MyQTLsummary.csv"`. We can set `method` as either `"Gprime"` or `"QTLseq"` depending on the type of analysis; `"Gprime"` will use `alpha` as FDR threshold and `"QTLseq"` will use the `interval` paramter, which should match one of the intervals calculated above.
